...
```bash
bash scripts/backup_db.sh --verify-restore
# 确认 backup file + .meta 文件都生成，restore_verified=true
```

> **实施门禁**：当前仓库的 `scripts/backup_db.sh` 仍只读取单个 `${MYSQL_DATABASE}`，且 `mysqldump` 尚未包含 `--source-data=2`。在依赖PITR坐标、执行跨Schema迁移或正式拆分 `control_db / collection_db / telemetry_db` 前，必须先升级备份脚本并完成隔离恢复测试。文档中的三库备份命令是目标实现，不能视为当前脚本已经具备的能力。
...
目标版 `backup_db.sh` 必须使用 `mysqldump --source-data=2`，在 dump 文件中写入当时的 binlog 坐标。当前仓库脚本尚未包含该参数，完成本节后方可依赖 dump 做 PITR。提取方式：
...
# 在 cron 中配置：0 */4 * * * bash /root/vllm_ascend_dashboard/scripts/archive_binlog.sh
...
| 备份 | 最后成功时间、文件大小、恢复校验状态 | `backup_db.sh` 输出 + cron |
...
# 同一个镜像，不同启动命令
# API 容器：
docker run ... ghcr.io/meng/vllm-dashboard:${VERSION} python -m app.main

# Scheduler 容器：
docker run ... ghcr.io/meng/vllm-dashboard:${VERSION} python -m app.scheduler
```

**当前状态**：Scheduler 已经是独立的 class（`app/services/scheduler.py`），拆出来主要是加一个 `__main__` 入口。
...
```bash
compose run --rm --no-deps --entrypoint /opt/venv/bin/python backend scripts/migrate_mysql_schema.py
```
...
${IMAGE_REGISTRY}/vllm-dashboard:${VERSION}
  ...
  ├─ python -m app.main           → API 服务
  ├─ python -m app.scheduler       → 调度器
  ├─ python -m app.collector       → 采集器
  └─ python scripts/migrate_mysql_schema.py  → 数据库迁移
```
