> **架构状态说明（2026-08）**：本文保留历史技术细节，当前可执行目录与角色边界以 `docs/current/架构演进与运维方案.md`、`backend/api`、`backend/scheduler`、`backend/collector`、`backend/infrastructure` 和 `database/` 为准。本文中的旧示例不应直接用于部署。
